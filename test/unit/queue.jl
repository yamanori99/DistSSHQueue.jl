    ...
    mktempdir() do d
        ...
        withenv(
            ...
            cd(jobdir) do
                ...
                @test occursin(DistSSHQueue.help_verb_line("status", "Snapshot of the store"), help)
                @test occursin(DistSSHQueue.help_verb_line("watch", "Live status"), help)
                @test occursin("Danger", help)
                @test occursin(DistSSHQueue.help_verb_line("teardown -y", "Stop serve and remove queue-host files"), help)
                @test length(collect(eachmatch(r"teardown -y", help))) == 1
                @test !occursin("status [-q]", help)
                @test !occursin("setup [--force]", help)
                @test occursin("enable", help)
                @test occursin("disable", help)
                @test occursin("fetch", help)
                @test occursin("Copy a finished Kit leaf", help)
                @test occursin("size", help)
                ...
                @test occursin("qhost:HOST teardown -y", td_h)
